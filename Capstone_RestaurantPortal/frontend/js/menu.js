...
async function loadMenuData() {
  ...
  if (!restaurantId) {
    hideSkeleton();
    showError(
      "No restaurant selected. Please go back and choose a restaurant.",
    );
    return;
  }
  showSkeleton();
  try {
    ...
    if (!restaurantRes.ok)
      throw new Error(`Restaurant not found (${restaurantRes.status})`);
    if (!categoriesRes.ok)
    ...
    hideSkeleton();
    showError(
      err.message || "Could not load menu. Please check your connection.",
    );
  }
}
...
function renderHero(restaurant) {
  ...
  document.getElementById("restaurantPhone").textContent =
    "📞 " + (restaurant.phone || "--");
  document.getElementById("restaurantDesc").textContent =
...
async function addToCart(itemId, itemName, price) {
  ...
  try {
    const resCurrent = await apiFetch("/api/cart");

    let cartCurr = null;
    let quantity = 1;

    if (resCurrent.ok) {
      cartCurr = await resCurrent.json();
      const existingItem = cartCurr.items
        ? cartCurr.items.find(
            (item) => item.menuItemId === itemId || item.id === itemId,
          )
        : null;
      if (existingItem) {
        quantity = existingItem.quantity + 1;
      }
    } else if (resCurrent.status !== 404) {
      const err = await resCurrent.text();
      throw new Error(err || `Server error ${resCurrent.status}`);
    }
